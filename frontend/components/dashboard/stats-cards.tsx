...
import { DollarSign, Receipt, Banknote, CreditCard, Coins } from 'lucide-react'
...
export function StatsCards({ data }: StatsCardsProps) {
    const totals = data.reduce(
        (acc, row) => ({
            ...
            tips: acc.tips + row.propina,
        }),
        { paid: 0, count: 0, cash: 0, card: 0, tips: 0 }
    )
    ...
    const stats = [
        ...
        { label: 'Propina', value: formatCurrency(totals.tips), icon: Coins, color: 'text-amber-500', tooltip: { model: 'pos.order.line', field: 'Producto ID 399', filter: "Suma de Líneas" } },
